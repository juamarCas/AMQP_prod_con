...
private:
    ...
    static void callback(struct ev_loop *loop, struct ev_timer *timer, int revents)
    {
        ...
        // publish a message
        self->_channel->publish("", self->_queue, "Hello");
    }
...
//    AMQP::Address address("amqps://guest:guest@localhost/");
    ...
    channel.declareQueue("myqueue").onSuccess([&connection, &channel, loop](const std::string &name, uint32_t messagecount, uint32_t consumercount) {
